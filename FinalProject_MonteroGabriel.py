...
#This program is to create a text based game in python.
...
#First we import random and time. Random we use in calculations to add a randeom element in leveling damage dealing

#Importing time to add a delay with text pop-ups and record the game time at the end
...
#Here is the mission objective. The player needs to slay the Dragon lord in order to win the game
...
#Here is the functionused to add colro to the printed text while advanince in the game.
...
#This segment is where we code thte classes, as well as the skills they have and the cost to use the skill
...
#Here we have a function to add a border arounrd text to help with reading
...
#Here is the function that defines the character, the classes from earlier will influence how stats are loaded in the game
...
#A shop. Just a place to buy potions and Eithers
...
#Skills that make the classes stronger. Higher levels grant stronger skills while there are some utility skills for buffing and debuffing.
...
#The combat loop. This will show how the battle is displayed as well as how each input affect the flow of combat
#Level up system is implemented near end
...
#Your typical enemy encounters. Earlier enemies are weak while later enemies scale to player level
#Enemy states, exp, and gold affected here
...
#The town function. Rest at in, go to shop, or fight enemies to then fight the final boss
...
def main():
    while True:
        ...
        # Create player
        boxed_text('Hello, adventurer! Please name your Hero')
